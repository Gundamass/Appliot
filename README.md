...
| 未知 ATS 认证适配 | 已实现，候选只读观察、确定性合成回放、AI 辅助审阅和人工认证 |
| 最终投递 | 明确禁止自动执行 |
...
- 已有认证提示包的页面继续走受控填写；未知或未匹配页面会进入 `awaiting_adapter_review`，认证前不会写入真实页面。
- 认证严格经过 `candidate -> replay_verified -> ai_reviewed -> human_reviewed -> certified`。确定性回放硬门禁失败时，AI 不能覆盖；AI 不可用时必须由人工明确确认降级路径。
- 修订会创建新的不可变 patch 版本并重新回放；发现指纹漂移、错写或回读回归时应退休旧版本。最终提交仍由系统双重禁止。

完整的生命周期、脱敏账本、调试原文保留和操作接口见 [`docs/ats-adapter-certification.md`](docs/ats-adapter-certification.md)。
...
docs/
  ats-adapter-certification.md  ATS 适配包认证与脱敏调试运行手册
