...
Program: historical application task recovery
Plan: docs/superpowers/plans/2026-09-09-historical-application-task-recovery.md
Branch start: 40e5e11
Execution: direct workspace on main, continuing the user-confirmed implementation sequence.
Task 1: complete (commits 40e5e11..193b92f; review clean after malformed-JSON rollback fix; focused API 29/29, API typecheck and diff check passed).
Task 2: complete (commits 193b92f..c38ea82; review clean after strict-body and in-flight replay fixes; contracts 204/204, API routes 43/43, contracts/API typecheck and diff check passed).
Task 3: complete (commit 6afb086; spec and quality approved; focused Web 39/39, Web typecheck and diff check passed).
Task 3 minor: terminal precedence has direct failed-state coverage but not separate cancelled/review_locked cases.
Task 3 minor: restart success is tested, while pending/no-navigation and rejection/no-navigation are established by implementation inspection rather than dedicated tests.
Task 4: complete (real database backed up at `.runtime/backups/application-recovery-20260909-203622/resume-assistant.sqlite`; historical Wondershare URL canonicalized; stale-checkpoint/latest-terminal-event projection made restartable; deterministic restart UUID audit false-positive fixed while standalone phone PII remains blocked; Tavily entry deduplication ignores fragments without removing application hash routes; real Wondershare restart reached `needs_questions`; fresh Mokahr task preserved the exact `#/job/a6cadf99-015c-42f6-a170-3252b540dae6/apply` route and reached `needs_questions`; both regression tasks cancelled before any submission; root workspace 141 API files/1284 API tests plus all package suites passed, root typecheck/build/diff check passed; one resource-contention timeout was isolated and passed with development services stopped).
Task 4 review follow-up: important finding fixed so stale persisted terminal events consistently suppress recovery commands and authorize terminal deletion as well as restart; focused routes 44/44 passed, serial API full suite 141 files/1284 tests passed, root typecheck/build/diff check passed. Default parallel API runs exposed pre-existing 5-second resource/order flakes in the Runtime restore and SkillSelector tests; each passed independently, and the same complete API suite passed with one worker without skipped tests or relaxed assertions.
