    ...
    async runUntilPause(taskId: string, initialSnapshot?: FormSnapshot): Promise<void> {
      ...
      const operation = (async () => {
        ...
        if (initialSnapshot !== undefined) {
          if (initialSnapshot.taskId !== taskId) throw new Error("application_snapshot_task_mismatch");
          dependencies.browser.seedObservation?.(taskId, initialSnapshot);
        }
        const runId = runIdFor(taskId);
        if (runId === undefined) {
...
function toApiState(value: ApplicationServiceSnapshot["value"]): "created" | "observing_page" | "waiting_for_login" | "needs_questions" | "awaiting_content_review" | "awaiting_challenge" | "filling" | "validating" | "navigating" | "review_locked" | "cancelled" | "failed" {
