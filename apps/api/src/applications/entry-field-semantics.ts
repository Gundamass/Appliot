...
type EntryKind = "education" | "projects" | "work";
...
export function deriveEntrySemanticHints(fields: FormField[]): FormField[] {
  const state: EntryState = { education: -1, projects: -1, work: -1, active: undefined };
...
function fieldFor(kind: EntryKind, label: string): string | undefined {
  const fields = kind === "projects" ? PROJECT_FIELDS : kind === "work" ? WORK_FIELDS : EDUCATION_FIELDS;
