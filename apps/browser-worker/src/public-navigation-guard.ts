...
export async function installPublicNavigationGuard(
  page: Pick<Page, "route" | "unroute">,
  resolveHostname: HostnameResolver = defaultResolveHostname
): Promise<() => Promise<void>> {
  const handler = createPublicNavigationGuardHandler(resolveHostname);
...
export function createPublicNavigationGuardHandler(
  resolveHostname: HostnameResolver = defaultResolveHostname
  ...
  return async (route) => {
    ...
    try {
      await validatePublicHttpsNavigation(request.url(), resolveHostname);
      await route.continue();
    } catch {
      await route.abort("blockedbyclient");
      throw new Error("unsafe_recruitment_redirect");
    }
  };
}

async function validatePublicHttpsNavigation(
  rawUrl: string,
...
function isPrivateAddress(value: string): boolean {
  ...
  if (kind === 6) {
    if (normalized === "::" || normalized === "::1" || normalized.startsWith("fc") || normalized.startsWith("fd")
      ...
      || normalized.startsWith("feb") || normalized.startsWith("ff") || normalized.startsWith("2001:db8")) return true;
