...
- 对“标识符参数的 ASCII 值或 ASCII 路径段后紧跟一段百分号编码自然语言”的高置信场景，恢复中文后缀到 `modelText`，并从 `url` 中剥离。查询参数只接受名称以 `id`、`uuid`、`token`、`code` 或 `key` 结尾且前缀值为 ASCII 标识符的情况；恢复文本至少包含四个汉字，并具有明确的中文句式开头或结尾，避免把姓名等合法参数值误切为正文。
- 中文搜索参数等非标识符参数保持完整，避免把合法 URL 内容误当用户话术。
- HTTP 与 HTTPS 链接统一计数；只允许恰好一个 HTTPS 链接进入模型，HTTP-only 或混合多链接输入均确定性降级，任何真实 URL 都不暴露给模型。
